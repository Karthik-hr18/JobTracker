...
const Register = () => {
    ...
    const submitForm = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await api.post("/user/userReg", user);
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 md:p-16">
                <motion.div className="w-full max-w-md bg-white p-8 sm:p-10 rounded-2xl shadow-sm border border-gray-100" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
                    <div className="mb-10 text-center sm:text-left">
                        ...
                        <p className="text-gray-500 text-[15px]">Start tracking your job applications today.</p>
                    </div>

                    <form onSubmit={submitForm} className="space-y-5">
                        <div>
                            <label className="block text-[14px] font-medium text-gray-900 mb-1.5">Username</label>
                            <div className="relative">
                                <User size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                                <input type="text" name="username" value={user.username} onChange={inputHandler} className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-[15px] rounded-lg focus:ring-black focus:border-black block pl-10 p-2.5 transition-all outline-none" placeholder="johndoe" required />
                            </div>
                        </div>
                    ...
                    <p className="text-center mt-8 text-[14px] text-gray-500">
                        Already have an account? <Link to="/userLogin" className="font-semibold text-black hover:underline">Sign in</Link>
                    </p>
                </motion.div>
            </div>
        </div>
